...
async def fetch_offer(base_url: str, timeout: float) -> StreamRequestBody:
  def get_offer() -> dict:
  ...
  return StreamRequestBody(**payload)


async def post_answer(base_url: str, answer_sdp: str, answer_type: str, timeout: float) -> None:
  def send_answer() -> None:
    resp = requests.post(join_url(base_url, "/answer"), json={"sdp": answer_sdp, "type": answer_type}, timeout=timeout)
    resp.raise_for_status()
...
async def run_once(args: argparse.Namespace) -> None:
  ...
  print(f"received offer cameras={','.join(offer.cameras or [offer.init_camera])}", flush=True)

  session = StreamSession(offer)
  try:
    answer = await session.get_answer()
    await post_answer(args.signaling_url, answer.sdp, answer.type, args.http_timeout)
    print("posted answer", flush=True)
    session.start()
