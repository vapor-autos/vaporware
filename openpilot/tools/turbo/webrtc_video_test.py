...
class WebrtcdConnectionProvider:
  def __init__(self, host: str, port: int, camera: str, enabled: bool = True):
...
async def run(args: argparse.Namespace) -> None:
  builder = WebRTCOfferBuilder(WebrtcdConnectionProvider(args.host, args.port, args.camera))
  ...
  try:
    ...
    track = stream.get_incoming_video_track(args.camera, buffered=False)
    ...
    while end_time is None or time.monotonic() < end_time:
      ...
      if now - last_log_time >= args.log_interval:
        ...
        print(
          f"frames={frames} fps={fps:.1f} size={frame.width}x{frame.height} pts={frame.pts} time={frame.time}",
          flush=True,
        )
        last_log_time = now
